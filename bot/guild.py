...
def get_guild_storage(guild_id):
    conn = sqlite3.connect(DB_NAME)
    cur = conn.cursor()
    cur.execute('SELECT id, template_id, level, rarity, upgrade_level, quantity FROM guild_storage WHERE guild_id = ?', (guild_id,))
    rows = cur.fetchall()
    conn.close()
    items = []
    for row in rows:
        ...
        if stats:
            stats['id'] = row[0]
            stats['quantity'] = row[5]
            items.append(stats)
    return items

def add_to_guild_storage(character_id, item_id):
    conn = sqlite3.connect(DB_NAME)
    cur = conn.cursor()
    ...
    cur.execute('INSERT INTO guild_storage (guild_id, template_id, level, rarity, upgrade_level, quantity) VALUES (?, ?, ?, ?, ?, ?)',
                (guild_id, template_id, level, rarity, upgrade_level, quantity))
    conn.commit()
    conn.close()
...
def remove_from_guild_storage(guild_id, storage_id, quantity, character_id):
    conn = sqlite3.connect(DB_NAME)
    cur = conn.cursor()
    cur.execute('SELECT template_id, level, rarity, upgrade_level, quantity FROM guild_storage WHERE id = ? AND guild_id = ?', (storage_id, guild_id))
    row = cur.fetchone()
    if not row:
        conn.close()
    ...
    conn.commit()
    conn.close()
    return True, f"Вы забрали {quantity} шт. со склада."